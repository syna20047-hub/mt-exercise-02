...
parser.add_argument('--logfile', type=str, default='',
                    help='path to save perplexity logs in TSV format')
...
def init_logfile():
    if args.logfile:
        ...
        with open(args.logfile, 'w', encoding='utf-8') as f:
            f.write('split\tepoch\tbatch\tloss\tppl\tdropout\n')


def append_log(split, epoch, batch, loss, ppl):
    if args.logfile:
        with open(args.logfile, 'a', encoding='utf-8') as f:
            f.write(f'{split}\t{epoch}\t{batch}\t{loss:.6f}\t{ppl:.6f}\t{args.dropout}\n')

# Set the random seed manually for reproducibility.
torch.manual_seed(args.seed)

# CPU is default, --accel only works if that attribute actually exists
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            append_log('train', epoch, batch, cur_loss, math.exp(cur_loss))
            total_loss = 0
            start_time = time.time()
        if args.dry_run:
            break
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss_eval = evaluate(train_data)
        append_log('train_epoch', epoch, -1, train_loss_eval, math.exp(train_loss_eval))
        ...
        append_log('valid', epoch, -1, val_loss, math.exp(val_loss))
        print('-' * 89)
...
append_log('test', 'final', -1, test_loss, math.exp(test_loss))
print('=' * 89)
